...
#Base de datos
setwd("~/R_sites/R-ladies/GGplotShiny-main/GGplotShiny-main/ggplot2_R_ladies_Morelia")
...
#1. Base del gráfico. Aquí utilizamos la función de ggplot con dos elementos básicos: 
# 1) la base de datos
# 2) los elementos estéticos. 
...
#2. Se agregan los elementos est?ticios aes(), en este caso las variables (x, y) 
# qu? queremos representar y agregamos el tipo de geometr?aa para representar los datos.
...
b<-ggplot(bd, aes(x=Diet, y = Mass, fill=Diet))+
    ...
    scale_fill_manual(values=cbPalette,labels=c("Carnivore", "Herbivore", "Omnivore"))
b
